...
from numpy import around, random, pi, sin, cos, sign, array
...
class Cart:                  
    def __init__ (self,             
                  ...
                  Mfric = 0.01, #1.0, # cart friction, N/m/s
                  Jfric = 0.1,#10.0, # friction coefficient on angular velocity, Nm/rad/s
                  g = 9.8, # gravity, m/s^2
                  L = 0.5, # half length of pend, m
                  umax = 10.0, # max cart force, N
    ...
    def Qreset(self):
        self.reset_state()
        return array([self.CartPosition, self.CartPositionD, self.angle, self.angleD])
    
    def Qstep(self, action):
        # self.Q += float(2*action-1)
        # self.Q += 0.1*(2*action-1)
        # if self.Q > 1.0:
        #     self.Q = 1.0
        # elif self.Q < -1.0:
        #     self.Q = -1.0
        self.ueff = 10.0*float(2*action-1)
        # self.ueff += 1.0*float(2*action-1)
        # if self.ueff > 10.0:
        #     self.ueff = 10.0
        # elif self.ueff < -10.0:
        #     self.ueff = -10.0
        self.update_state(dt = 0.02)
        state = (self.CartPosition, self.CartPositionD, self.angle, self.angleD)
        
        done = bool(
            self.angle < -2*self.angle_safety_limit*(pi/180.0)
            or self.angle > 2*self.angle_safety_limit*(pi/180.0)
        )

        reward = 1.0

        return array(state), reward, done, {}
